...
加简介summary要改数据库和createPost,categoryName该Mapper用Left塞进VO

现在刚加summary，还需要改代码层面

场景二：在浏览页，ById查询到帖子内容和标题和分区和点赞数等信息，排除简介和封面图

需要新的VO和Mapper感觉

场景三：被举报的帖子被管理员查询，照样按照首页的展示形式(标题加简介，点击后展示内容;写到举报时再根据举报方式决定这里查询用一个列表记录所有id单个查询，还是post又加一个状态去分页查询，哦想起来了举报有表的，对照着表上去单个查询好了)