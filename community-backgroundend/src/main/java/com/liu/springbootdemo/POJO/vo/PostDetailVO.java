...
@Data
@Schema(name = "单帖子具体展示视图对象 VO", description = "比summaryVO多content、发布者IP等具体信息") //TODO:前提是该点有新技术点不然感觉算业务：之后在该页面前端记得还要请求User的帖子数、关注、等级之类的信息；不知道关注这种算不算亮点不算就不搞了这样还需要user多几个关注接口和表要加上同样的冗余字段
public class PostDetailVO {

    private Long id;        //帖子id 让前端好找去跳转
    private String title;   //外显标题
    private String summary; // 帖子摘要/简介
    private Long userId;    //发表用户id 给前端做用户主页链接等展示查询用 26.3.24为了前端加
